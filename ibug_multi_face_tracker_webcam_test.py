    ...
    if len(sys.argv) < 2:
        ...
        while True:
            ...
            if ret:
                ...
                face_ids = [face['id'] for face in tracked_faces]
                if len(face_ids) == 0:
                    print("Frame #%d: no face is tracked." % frame_number)
                elif len(face_ids) == 1:
                    print("Frame #%d: face #%d is tracked." % (frame_number, face_ids[0]))
                else:
                    print("Frame #%d: face #%d" % (frame_number, face_ids[0]) +
                          ", #".join([str(face_id) for face_id in face_ids[1:-2]]) +
                          " and #%d" % face_ids[-1] + " are tracked.")
                ...
                for face in tracked_faces:
                    ...
                    cv2.putText(frame, "Face #%d" % face['id'], text_origin, cv2.FONT_HERSHEY_DUPLEX,
